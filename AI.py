  ...
  while hand_size > 1:
    ...
    for cut in range(num_players):
      ...
      if new_cutee == cutee or revealed[cutee] >= hand_size:
        return np.array(observations), np.array(actions), np.full(np.array(actions).shape, -1)
      else: cutee = new_cutee
      randy = randint(1, hand_size - revealed[cutee])
      if bombs[cutee] == 1 and randy == hand_size - revealed[cutee]:
        return np.array(observations), np.array(actions), np.zeros(np.array(actions).shape)
...
def train_policy(actor, games_per_epoch, policy_optimizer):
  all_observations = []
  all_actions = []
  all_returns = []
...
def main():
  ...
  for epoch in range(num_epochs):
    loss = train_policy(actor, games_per_epoch, policy_optimizer)
