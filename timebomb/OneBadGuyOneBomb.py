...
def PlayAuto(num_players=4, initial_hand_size=5, verbosity=2):
  ...
  while hand_size > 1:
    ...
    # Deal the bomb to a random hand, then the wires uniformly among the remaining
    # non-bomb slots (slot-uniform / multivariate hypergeometric; the bomb hand offers
    # H-1 wire slots).
...
def Play(players=["Alice", "Bob", "Clara", "Darryl"], initial_hand_size=5):
  ...
  while hand_size > 1:
    ...
    print("d:", declarations)
    # Calculate probabilities
    if not cons.declarations_feasible(declarations, hand_size, active_wires, 1, 1):
